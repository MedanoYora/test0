...
async def new_connect(input_message,output_message):
    ...
    if command["action"]=="autorization":
        ...
        if command["token"]+"\n" in all_token:
            ...
            user_id=cursor.lastrowid # отримаэмо id щойноствореного 
            time_now=int(time.time())
            cursor.execute("INSERT INTO people (id_user, count, last_update) VALUES (%s, %s,%s)", (user_id,start_resource["people"]))
            cursor.execute("INSERT INTO tree (id_user, count, last_update) VALUES (%s, %s, %s)", (user_id,start_resource["tree"]))
            cursor.execute("INSERT INTO stone (id_user, count, last_update) VALUES (%s, %s, %s)", (user_id,start_resource["stone"]))
            cursor.execute("INSERT INTO food (id_user, count, last_update) VALUES (%s, %s, %s)", (user_id,start_resource["food"]))
            cursor.execute("INSERT INTO iron (id_user, count, last_update) VALUES (%s, %s, %s)", (user_id,start_resource["iron"]))
            cursor.execute("INSERT INTO gold (id_user, count, last_update) VALUES (%s, %s, %s)", (user_id,start_resource["gold"]))
            cursor.execute("INSERT INTO oil (id_user, count, last_update) VALUES (%s, %s, %s)", (user_id,start_resource["oil"]))
            connection.commit()
            #for i in range(1000):
            #    print(i)
...
async def main():
    server = await asyncio.start_server(new_connect, '192.168.50.206',2458)
    async with server:
