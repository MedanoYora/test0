...
# Вставка даних в таблицю
command = "INSERT INTO {0} (name, mining_speed, resource_multiplier, people, food, tree, stone, oil, iron, gold ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)".format(resource)
...
cursor.execute(command, data)
data = ("oil well", 1000, 1, 2, 1, 10, 5, 0, 0, 50)
...
cursor.execute("""CREATE TABLE IF NOT EXISTS {0} (
    ...
    id_user INT,
    FOREIGN KEY (id_user)  REFERENCES {1}(id_user) ON DELETE CASCADE ON UPDATE CASCADE                   
)""".format("mine_stone",users)) # шахта каменю
...
cursor.execute("""CREATE TABLE IF NOT EXISTS {0} (
    ...
    id_user INT,
    FOREIGN KEY (id_user)  REFERENCES {1}(id_user) ON DELETE CASCADE ON UPDATE CASCADE                   
)""".format("mine_iron",users)) # шахта заліза
...
cursor.execute("""CREATE TABLE IF NOT EXISTS {0} (
    ...
    id_user INT,
    FOREIGN KEY (id_user)  REFERENCES {1}(id_user) ON DELETE CASCADE ON UPDATE CASCADE                   
)""".format("mine_gold",users)) # шахта золота
